...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        ...
        printf("********** Global Sorted Array 1 - Size: %d **********\n", size_g_sortedIntegerArray1);
        printArray(g_sortedIntegerArray1, size_g_sortedIntegerArray1);

        printf("********** Global Sorted Array 2 - Size: %d **********\n", size_g_sortedIntegerArray2);
        printArray(g_sortedIntegerArray2, size_g_sortedIntegerArray2);
    ...
    if (rank == 0) {
        ...
        for (int source = size - 1; source > 0; source--) {
            ...
            // Update total merged array size
            totalMergedArraySize += receivedMergedArraySize;

            // Reallocate memory for merged array
            mergedArray = (int*)realloc(mergedArray, totalMergedArraySize * sizeof(int));

            // Add received array values to final merged array
            for (int i = 0; i < receivedMergedArraySize; i++) {
                mergedArray[offset + i] = receivedMergedArray[i];
            }

            // Update the offset
            offset += receivedMergedArraySize;

        }

        // Update total merged array size
        totalMergedArraySize += size_merged_array;
        mergedArray = (int*)realloc(mergedArray, totalMergedArraySize * sizeof(int));

        // Add values from rank 0 merged array to final merged array
        for (int i = 0; i < size_merged_array; i++) {
                mergedArray[offset + i] = merged_array[i];
        }

        // Print final merged array
        printf("********** Final merged array **********\n");
        printArray(mergedArray, totalMergedArraySize);
